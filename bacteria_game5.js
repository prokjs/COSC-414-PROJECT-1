...
var InitDemo = function() {
	...
	var poisonSectionSize = [];
	var poisonAngleStart = [];
	...
	var tick = function() {
		...
		for (i = 0; i <= currentBacteriaNumber; i++) {
			...
			if(poison == true){
				console.log(poisonAngleStart[i]);
				if(poisonSectionSize[i] <= 2160){
					poisonSectionSize[i] = updateSectionSize(poisonSectionSize[i], i);
				}
				if(poisonSectionSize[i] >= 3000){
		...
		for (i = 0; i <= currentBacteriaNumber; i++) {
			...
			//Creates a poison propogating from the vertex of a circle that was poisoned
			if(poison == true){
				createCircle(sectionSize[i],angleStart[i],1,360,0.9,colorStart[i],i);	
				drawCircles(circleVertices,n);
				createCircle(sectionSize[i],angleStart[i],-1,360,0.9,colorStart[i],i);	
				drawCircles(circleVertices,n);
				createCircle(poisonSectionSize[i], poisonAngleStart[i], 1, 360, 0.9, poisonColor, i);
				drawCircles(circleVertices, n);
				createCircle(poisonSectionSize[i], poisonAngleStart[i], -1, 360, 0.9, poisonColor, i);
				drawCircles(circleVertices, n);
			}
